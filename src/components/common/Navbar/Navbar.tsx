    ...
    <header className="sticky top-0 z-40 w-full glass-nav transition-all">
          ...
          {isAuthenticated && user ? (
            <div className="relative">
              ...
              >
                ...
                />
                <span className="text-xs font-medium text-foreground max-w-[100px] truncate">{user.username}</span>
              </button>
              <AnimatePresence>
                {userDropdownOpen && (
                  <motion.div
                  ...
                  >
                    ...
                    <Link to="/history" onClick={closeMenus} className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-accent/10 rounded-lg transition-colors">
                      <Ticket className="w-4 h-4 text-amber-400" /> My Bookings
                    </Link>
                    <Link to="/settings" onClick={closeMenus} className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-accent/10 rounded-lg transition-colors">
                      <Settings className="w-4 h-4 text-muted-foreground" /> Settings
                    </Link>
                    <div className="border-t border-border my-1 pt-1">
                      <button onClick={() => { logout(); closeMenus(); }} className="w-full text-left flex items-center gap-2 px-3 py-2 text-xs text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
            ...
            <div className="flex items-center gap-2">
              <Link to="/login" className="px-4 py-2 text-xs font-semibold text-foreground hover:text-[#E50914] transition-colors">Sign In</Link>
              <Link to="/register" className="px-4 py-2 rounded-lg bg-[#E50914] hover:bg-[#ff1f2d] text-white text-xs font-bold tracking-wide transition-all shadow-md shadow-[#E50914]/30">Join Now</Link>
            </div>
          )}
        </div>

        <div className="flex md:hidden items-center gap-2">
          <button onClick={toggleTheme} title={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/10">
            {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
          </button>
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/10">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={closeMenus} className="block px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/10">
                {link.name}
              </Link>
            ))}
            {canViewAdminPanel && (
            ...
            <div className="border-t border-border pt-3 flex flex-col gap-2">
              {isAuthenticated ? (
                <button onClick={() => { logout(); closeMenus(); }} className="w-full text-left px-3 py-2 text-sm font-medium text-rose-400">Sign Out ({user?.username})</button>
              ) : (
                <Link to="/login" onClick={closeMenus} className="w-full text-center py-2.5 rounded-lg bg-[#E50914] text-white text-xs font-bold">Sign In</Link>
              )}
            </div>
          </motion.div>
        )}
