...
export const MovieCard: React.FC<MovieCardProps> = ({ movie, className = '', onClick, variant = 'default' }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={handleClick}
      className={cn('movie-card group', variant === 'home' && 'home-movie-card', className)}
      role="article"
    ...
    >
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-card">
        ...
        <img
          ...
          className={cn('movie-card-poster transition-opacity duration-300', posterLoaded ? 'opacity-100' : 'opacity-0')}
          loading="lazy"
        />
      ...
      <div className={cn('mt-3 flex flex-col gap-1', variant === 'home' && 'mt-4')}>
        {variant === 'home' && <div className="flex items-center gap-2 text-xs text-white/60">
        ...
        {variant !== 'home' && <div className="flex items-center gap-2 text-xs text-white/50">
