...
import { ArrowRight, Bookmark, Check, ChevronDown, CircleAlert, MapPin, Popcorn, RefreshCw, Search, Ticket, X } from 'lucide-react';
...
export function OffersPage() {
  ...
  useEffect(() => { if (isFoodPage) void fetchCinemas(); }, [fetchCinemas, isFoodPage]);
...
>>>>>>> Stashed changes
        ...
        <section id="food-menu" aria-labelledby="menu-title" className="scroll-mt-32 pt-10"><div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-primary">{isFoodPage ? 'Fresh at the concessions counter' : 'Special offers'}</p><h2 id="menu-title" className="mt-1 text-3xl font-black tracking-tight text-foreground">{isFoodPage ? 'Food & drinks' : 'Promotions'}</h2></div>{isAuthenticated && !loading && !loadFailed && <button type="button" onClick={() => setRefresh((value) => value + 1)} className={`inline-flex min-h-10 items-center gap-2 self-start rounded-lg border border-border bg-card px-3 text-xs font-semibold text-foreground hover:bg-accent ${FOCUS}`}><RefreshCw className="h-3.5 w-3.5" />Refresh</button>}</div>
...
=======
          {!isAuthenticated ? <div className="rounded-xl border border-border bg-card px-6 py-12 text-center"><Coffee className="mx-auto mb-4 h-10 w-10 text-primary" strokeWidth={1.4} /><h3 className="text-xl font-bold text-foreground">Sign in to see the food menu</h3><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">Browse available snacks, compare prices, and save your favourites for your next movie night.</p><Link to={`/login?redirect=${encodeURIComponent(`${pathname}${search}`)}`} className={`${PRIMARY_BUTTON} mt-6`}>Sign in to browse <ArrowRight className="h-4 w-4" /></Link><p className="mt-4 text-xs text-muted-foreground">You can explore cinema locations without signing in.</p></div>
            : loading ? <div role="status" aria-label="Loading food menu" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((id) => <div key={id} className="h-64 rounded-xl border border-border bg-card motion-safe:animate-pulse" />)}</div>
              : loadFailed ? <div role="alert" className="rounded-xl border border-border bg-card p-10 text-center"><CircleAlert className="mx-auto mb-4 h-8 w-8 text-primary" /><h3 className="text-lg font-bold text-foreground">We couldn't load the food menu</h3><p className="mt-2 text-sm text-muted-foreground">Try again in a moment to see current prices and availability.</p><button type="button" onClick={() => setRefresh((value) => value + 1)} className={`${PRIMARY_BUTTON} mt-6`}><RefreshCw className="h-4 w-4" />Try again</button></div>
                : <><div className="mb-5 space-y-3 rounded-xl border border-border bg-card p-3"><div className="flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3.5 top-3.5 h-4 w-4 text-muted-foreground" /><label className="sr-only" htmlFor="menu-search">Search food and drinks</label><input id="menu-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search popcorn, drinks, combos…" className={`h-11 w-full rounded-lg border border-border bg-background pl-10 pr-10 text-sm text-foreground placeholder:text-muted-foreground ${FOCUS}`} />{query && <button type="button" aria-label="Clear search" onClick={() => setQuery('')} className={`absolute right-1 top-1 rounded-lg p-2.5 text-muted-foreground ${FOCUS}`}><X className="h-4 w-4" /></button>}</div><label className="flex h-11 items-center gap-2 rounded-lg border border-border bg-background px-3 text-xs text-muted-foreground">Sort by<select aria-label="Sort by" value={sort} onChange={(event) => setSort(event.target.value)} className={`min-w-0 bg-transparent py-2 text-sm text-foreground ${FOCUS}`}><option value="name" className="bg-card text-foreground">Name</option><option value="price-low" className="bg-card text-foreground">Price: low to high</option><option value="price-high" className="bg-card text-foreground">Price: high to low</option></select></label></div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => setCategoryId('all')} aria-pressed={categoryId === 'all'} className={`rounded-full border px-3.5 py-2 text-xs font-semibold ${categoryId === 'all' ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-foreground hover:bg-accent'} ${FOCUS}`}>All food & drinks</button>{menuCategories.map((category) => <button type="button" key={category.id} onClick={() => setCategoryId(String(category.id))} aria-pressed={categoryId === String(category.id)} className={`rounded-full border px-3.5 py-2 text-xs font-semibold ${categoryId === String(category.id) ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-foreground hover:bg-accent'} ${FOCUS}`}>{category.name}</button>)}<button type="button" onClick={() => setSavedOnly((value) => !value)} aria-pressed={savedOnly} className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-xs font-semibold sm:ml-auto ${savedOnly ? 'border-primary bg-primary/15 text-primary' : 'border-border bg-card text-foreground hover:bg-accent'} ${FOCUS}`}><Bookmark className="h-3.5 w-3.5" fill={savedOnly ? 'currentColor' : 'none'} />Saved ({savedCount})</button></div></div><p className="mb-4 text-xs text-muted-foreground" aria-live="polite">{visibleProducts.length} {visibleProducts.length === 1 ? 'item' : 'items'} to explore</p>{visibleProducts.length === 0 ? <div className="rounded-xl border border-dashed border-border px-6 py-14 text-center"><Popcorn className="mx-auto mb-4 h-8 w-8 text-muted-foreground" /><h3 className="text-lg font-bold text-foreground">{products.length === 0 ? 'The menu is being prepared' : savedOnly && savedCount === 0 ? 'Start your movie-night shortlist' : 'No matching snacks or drinks'}</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">{products.length === 0 ? 'No food or drinks are available right now. You can still book your next film.' : 'Try another category or a different search.'}</p>{products.length > 0 ? <button type="button" onClick={resetFilters} className={`${PRIMARY_BUTTON} mt-5`}>Show all food & drinks</button> : <Link to="/cinemas" className={`${PRIMARY_BUTTON} mt-5`}>Find a showtime <ArrowRight className="h-4 w-4" /></Link>}</div> : <div className="grid items-start gap-4 sm:grid-cols-2 lg:grid-cols-3">{visibleProducts.map((product) => { const category = categoryMap.get(product.productCategoryId); const saved = savedIds.includes(product.id); return <article key={product.id} className="overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-md"><div className="relative aspect-[1.45/1] bg-muted"><SnackImage key={`${product.id}:${product.imageUrl}`} name={product.name} category={imageCategory(category?.name || '')} src={product.imageUrl} /><button type="button" onClick={() => toggleSaved(product)} aria-label={`${saved ? 'Unsave' : 'Save'} ${product.name}`} aria-pressed={saved} className={`absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background/80 text-foreground backdrop-blur-md ${saved ? 'text-primary' : 'hover:text-primary'} ${FOCUS}`}><Bookmark className="h-4 w-4" fill={saved ? 'currentColor' : 'none'} /></button></div><div className="p-4"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-primary">{category?.name || 'Concessions'}</p><div className="mt-1 flex items-start justify-between gap-3"><h3 className="text-base font-bold leading-6 tracking-tight text-foreground">{product.name}</h3><p className="shrink-0 font-mono text-base font-bold text-primary">{formatCurrency(product.price)}</p></div><p className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground"><Check className="h-3.5 w-3.5 text-emerald-500" />Available to add during booking</p><details className="group mt-4 border-t border-border pt-3"><summary className={`flex cursor-pointer list-none items-center justify-between rounded text-xs font-bold text-foreground/80 [&::-webkit-details-marker]:hidden ${FOCUS}`}>Item details <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-open:rotate-180" /></summary><div className="pt-3 text-xs leading-6 text-muted-foreground">{category?.description && <p className="mb-2">{category.description}</p>}<p>Choose a showtime and your seats, then add this item during booking.</p><Link to="/cinemas" className={`mt-3 inline-flex items-center gap-2 text-sm font-bold text-primary ${FOCUS}`}>Find a showtime <ArrowRight className="h-4 w-4" /></Link></div></details></div></article>; })}</div>}</>}
>>>>>>> Stashed changes
        </section>
        <div className="mt-12 flex flex-col justify-between gap-4 border-t border-border pt-7 sm:flex-row sm:items-center"><div><h2 className="font-bold text-foreground">Ready for the big screen?</h2><p className="mt-1 text-sm text-muted-foreground">Find your cinema, choose a film, and add the finishing touches.</p></div><Link to="/cinemas" className={PRIMARY_BUTTON}>Explore showtimes <ArrowRight className="h-4 w-4" /></Link></div>
      </main>
