    ...
    <div className="min-h-screen pb-20 text-white">
        ...
        <div className="container-main py-8 sm:py-10">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm leading-relaxed text-white/50">Choose your cinema, find a screening, and make it a movie night.</p>
            </div>
            <button type="button" onClick={retry} disabled={busy} className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2.5 text-xs font-semibold hover:bg-white/5 disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${busy ? 'animate-spin' : ''}`} /> Refresh listings</button>
          </div>
          <div className="mt-8 flex items-center justify-between gap-3">
            <h2 className="flex items-center gap-2 text-sm font-bold"><MapPin className="h-4 w-4 text-[var(--primary)]" /> Choose a cinema <span className="font-normal text-white/50">{cinemas.length > 0 && `(${cinemas.length})`}</span></h2>
            <button type="button" onClick={() => chooseCinema('ALL')} aria-pressed={selectedCinemaId === 'ALL'} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${selectedCinemaId === 'ALL' ? 'border-[var(--primary)] bg-[var(--primary)]/10 text-[var(--primary)]' : 'border-white/10 text-white/50 hover:text-white'}`}>All cinemas</button>
          </div>
          {cinemaError ? <div role="alert" className="mt-4 rounded-xl border border-red-500/30 p-4 text-sm"><p>{cinemaError}</p><button type="button" onClick={() => void fetchCinemas(true)} className="mt-2 font-semibold text-[var(--primary)] underline">Retry locations</button></div> : cinemasLoading && !cinemas.length ? <p role="status" className="py-6 text-sm text-white/50">Loading cinema locations…</p> : !cinemas.length ? <p className="py-6 text-sm text-white/50">Cinema locations will appear here when they are available.</p> : (
          ...
          {locationError && <p role="status" className="mt-3 text-xs text-white/50">Map details are temporarily unavailable. <button onClick={() => void fetchCinemas(true)} className="underline">Retry</button></p>}
          {selectedCinema && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-white/[0.03] px-4 py-3">
            <p className="flex items-center gap-2 text-xs text-white/50"><MapPin className="h-4 w-4 shrink-0 text-[var(--primary)]" />{selectedCinema.address || selectedCinema.locationName || selectedCinema.name}</p>
      ...
      <section className="container-main mt-7">
        <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div><h2 className="text-xl font-bold tracking-tight">{activeDate === today ? 'Today' : dateLabel(activeDate, { weekday: 'long', month: 'short', day: 'numeric' })}<span className="ml-2 text-sm font-normal text-white/50">at {selectedCinema?.name || 'all cinemas'}</span></h2><p className="mt-1 flex items-center gap-1.5 text-xs text-white/50"><Clock3 className="h-3.5 w-3.5" /> Cambodia time (UTC+7) · Upcoming screenings only</p></div>
          <label className="relative w-full sm:max-w-xs"><Search className="absolute left-3 top-3 h-4 w-4 text-white/50" /><input aria-label="Search movies or genres" placeholder="Search movies or genres" value={search} onChange={(event) => setSearch(event.target.value)} className="w-full rounded-lg border border-white/10 bg-white/[0.02] py-2.5 pl-9 pr-9 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)]" />{search && <button type="button" onClick={() => setSearch('')} aria-label="Clear movie search" className="absolute right-3 top-3 text-white/50"><X className="h-4 w-4" /></button>}</label>
        </div>
        <button type="button" onClick={() => setMobileFilterOpen(!mobileFilterOpen)} aria-expanded={mobileFilterOpen} aria-controls="mobile-showtime-filters" className="mb-4 flex w-full items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-4 py-3 text-sm lg:hidden"><span className="flex items-center gap-2"><Filter className="h-4 w-4 text-[var(--primary)]" />Format & time{activeFiltersCount > 0 && ` (${activeFiltersCount})`}</span><span>{mobileFilterOpen ? 'Hide' : 'Show'}</span></button>
