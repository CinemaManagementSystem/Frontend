...
export const ShowtimeEmptyState = ({ type, cinemaName, selectedDate, errorMessage, tomorrowDateStr, onResetFilters, onSelectTomorrow, onRetry, onShowAllCinemas }: ShowtimeEmptyStateProps) => {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-6 py-14 text-center" role={type === 'ERROR' ? 'alert' : 'status'}>
      ...
      <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-white/50">{type === 'ERROR' ? errorMessage || 'Please try refreshing the listings.' : type === 'FILTER_EMPTY' ? `Try a different movie, format, or time at ${cinemaName} on ${formatDay(selectedDate)}.` : `There are no bookable screenings at ${cinemaName} on ${formatDay(selectedDate)}. Choose another date or explore other cinemas.`}</p>
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        ...
        {type === 'NO_SHOWTIMES' && onShowAllCinemas && <button type="button" onClick={onShowAllCinemas} className="rounded-lg border border-white/10 px-5 py-2.5 text-sm font-semibold hover:bg-white/5">Explore all cinemas</button>}
      </div>
    </div>
  );
};
